...
export function Experiencia() {
  return (
    <div className="text-white relative">
      ...
      <section className="relative z-10 py-16 px-6 md:px-20 max-w-6xl">
        <motion.div
        ...
        >
          <h2 className="text-4xl md:text-6xl font-bold mb-2">Experiência Profissional</h2>
          <p className="text-zinc-400 text-sm md:text-md font-semibold mb-3">
            Os marcos profissionais que moldaram minha carreira.
          </p>
          <div className="w-20 md:w-50 h-1 bg-blue-600 rounded-full mb-14" />
        </motion.div>

        <motion.div
          className="flex-1 w-full lg:text-left relative flex flex-col gap-6 mb-20"
          variants={container}
          initial="hidden"
          whileInView="show"
          viewport={{ once: true }}
        >
          ...
          {experiences.map((exp, i) => (
            ...
            >
              ...
              <div className="w-full md:max-w-[800px] bg-zinc-900/60 border border-zinc-800 rounded-xl p-5 md:p-7 transition-all duration-300 hover:border-blue-500/50 hover:bg-zinc-900/80 hover:shadow-[0_4px_24px_rgba(59,130,246,0.08)]">

                <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                  <h3 className="font-bold text-white text-base">{exp.role}</h3>
                  <div className="flex items-center gap-2">
                    {exp.endIsPresent && (
                      <span className="text-[10px] md:text-xs font-semibold text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-md">
                        Atual
                      </span>
                    )}
                    <span className="text-[10px] md:text-xs font-semibold text-blue-300 px-2.5 py-1 rounded-md whitespace-nowrap">
                ...
                <div className="flex flex-wrap gap-2">
                  {exp.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[10px] md:text-xs text-zinc-400 border border-zinc-700 bg-zinc-800/60 px-3 py-1 rounded-md"
                    >
                      {tag}
                    </span>
                  ))}
